...
const PersonDashboard = () => {
  ...
  const addComplaint = async () => {
    ...
    console.log(desc);
    const data = await fetch("http://localhost:4500/user/generate_compalint", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
  ...
  const getRecentComplaints = async () => {
    ...
    const data = await fetch("http://localhost:4500/user/recent_complaint", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
  ...
  return (
    <>
      ...
      <div className="container-fluid py-4 complaint_container">
        ...
        <div
          class="modal fade"
          id="ComplaintCreateModal"
          tabindex="-1"
          aria-labelledby="ComplaintCreateModalLabel"
          aria-hidden="true"
        >
                      ...
                      >
                        ...
                        {categories ? (
                          categories.map((category, index) => {
                            return (
                              <option key={index} value={category.cid}>
                                {category.category}
                              </option>
                            );
                          })
        ...
        <div className="d-flex justify-content-between align-items-md-center flex-md-row flex-column align-items-start">
          <h5 className="my-4">Recent Complaints</h5>
          <Link to="/my_complaints" className="btn btn-primary">
            See all complaints
          </Link>
        </div>
